...
class Migration(SchemaMigration):
    ...
    def forwards(self, orm):
        # Adding field 'Complaints.handler'
        db.add_column(u'SmartDataApp_complaints', 'handler',
                      self.gf('django.db.models.fields.related.ForeignKey')(to=orm['auth.User'], null=True),
                      keep_default=False)

        # Removing M2M table for field handler on 'Complaints'
        db.delete_table(db.shorten_name(u'SmartDataApp_complaints_handler'))


        # Renaming column for 'Complaints.author' to match new field type.
        db.rename_column(u'SmartDataApp_complaints', 'author_id', 'author')
        # Changing field 'Complaints.author'
        db.alter_column(u'SmartDataApp_complaints', 'author', self.gf('django.db.models.fields.CharField')(max_length=250, null=True))
        # Removing index on 'Complaints', fields ['author']
        db.delete_index(u'SmartDataApp_complaints', ['author_id'])


    def backwards(self, orm):
        # Adding index on 'Complaints', fields ['author']
        db.create_index(u'SmartDataApp_complaints', ['author_id'])

        # Deleting field 'Complaints.handler'
        db.delete_column(u'SmartDataApp_complaints', 'handler_id')

        # Adding M2M table for field handler on 'Complaints'
        m2m_table_name = db.shorten_name(u'SmartDataApp_complaints_handler')
        db.create_table(m2m_table_name, (
            ('id', models.AutoField(verbose_name='ID', primary_key=True, auto_created=True)),
            ('complaints', models.ForeignKey(orm[u'SmartDataApp.complaints'], null=False)),
            ('profiledetail', models.ForeignKey(orm[u'SmartDataApp.profiledetail'], null=False))
        ))
        db.create_unique(m2m_table_name, ['complaints_id', 'profiledetail_id'])


        # User chose to not deal with backwards NULL issues for 'Complaints.author'
        raise RuntimeError("Cannot reverse this migration. 'Complaints.author' and its values cannot be restored.")
        
        # The following code is provided here to aid in writing a correct migration
        # Renaming column for 'Complaints.author' to match new field type.
        db.rename_column(u'SmartDataApp_complaints', 'author', 'author_id')
        # Changing field 'Complaints.author'
        db.alter_column(u'SmartDataApp_complaints', 'author_id', self.gf('django.db.models.fields.related.ForeignKey')(to=orm['auth.User']))
