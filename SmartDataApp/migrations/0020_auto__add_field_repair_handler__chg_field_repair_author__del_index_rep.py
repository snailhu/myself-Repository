...
class Migration(SchemaMigration):
    ...
    def forwards(self, orm):
        # Adding field 'Repair.handler'
        db.add_column(u'SmartDataApp_repair', 'handler',
                      self.gf('django.db.models.fields.related.ForeignKey')(to=orm['auth.User'], null=True),
                      keep_default=False)

        # Removing M2M table for field handler on 'Repair'
        db.delete_table(db.shorten_name(u'SmartDataApp_repair_handler'))


        # Renaming column for 'Repair.author' to match new field type.
        db.rename_column(u'SmartDataApp_repair', 'author_id', 'author')
        # Changing field 'Repair.author'
        db.alter_column(u'SmartDataApp_repair', 'author', self.gf('django.db.models.fields.CharField')(max_length=250, null=True))
        # Removing index on 'Repair', fields ['author']
        db.delete_index(u'SmartDataApp_repair', ['author_id'])


    def backwards(self, orm):
        # Adding index on 'Repair', fields ['author']
        db.create_index(u'SmartDataApp_repair', ['author_id'])

        # Deleting field 'Repair.handler'
        db.delete_column(u'SmartDataApp_repair', 'handler_id')

        # Adding M2M table for field handler on 'Repair'
        m2m_table_name = db.shorten_name(u'SmartDataApp_repair_handler')
        db.create_table(m2m_table_name, (
            ('id', models.AutoField(verbose_name='ID', primary_key=True, auto_created=True)),
            ('repair', models.ForeignKey(orm[u'SmartDataApp.repair'], null=False)),
            ('profiledetail', models.ForeignKey(orm[u'SmartDataApp.profiledetail'], null=False))
        ))
        db.create_unique(m2m_table_name, ['repair_id', 'profiledetail_id'])


        # User chose to not deal with backwards NULL issues for 'Repair.author'
        raise RuntimeError("Cannot reverse this migration. 'Repair.author' and its values cannot be restored.")
        
        # The following code is provided here to aid in writing a correct migration
        # Renaming column for 'Repair.author' to match new field type.
        db.rename_column(u'SmartDataApp_repair', 'author', 'author_id')
        # Changing field 'Repair.author'
        db.alter_column(u'SmartDataApp_repair', 'author_id', self.gf('django.db.models.fields.related.ForeignKey')(to=orm['auth.User']))
